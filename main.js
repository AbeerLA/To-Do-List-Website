...
// Add New Column Functionality (Custom Columns)
let newColumnInput = document.getElementById("newColumnInput");
let addColumnButton = document.getElementById("addColumnButton");
addColumnButton.onclick = function () {
  let columnName = newColumnInput.value.trim();
  if (columnName) {
...
// Function to create a new column (custom) that opens the edit column modal
function addNewColumn(columnName) {
  let kanbanBoard = document.querySelector(".kanban-board");
  let newColumn = document.createElement("div");
  newColumn.classList.add("column", columnName.toLowerCase());
  newColumn.setAttribute("data-category", columnName.toUpperCase());

  let header = document.createElement("h3");
  header.innerHTML = `<span class="column-title">${columnName.toUpperCase()}</span>`;
  newColumn.appendChild(header);

  // For custom columns, clicking the header opens the edit column modal
  header.addEventListener("click", function () {
    openEditColumnModal(newColumn);
  });

  kanbanBoard.insertBefore(newColumn, document.getElementById("newColumn"));
  setupDragAndDrop();
}

// Function to open the edit column modal (styled like the edit task modal)
function openEditColumnModal(columnElement) {
  currentEditColumn = columnElement;
  let currentName = columnElement.getAttribute("data-category");
  document.getElementById("editColumnInput").value = currentName;
  document.getElementById("editColumnModal").classList.remove("hidden");
}

// Event listener for saving edited column changes via the modal
document.getElementById("saveEditedColumn").onclick = function () {
  let newName = document.getElementById("editColumnInput").value.trim();
  if (newName) {
    let oldName = currentEditColumn.getAttribute("data-category");
    currentEditColumn.querySelector(".column-title").textContent = newName.toUpperCase();
    currentEditColumn.setAttribute("data-category", newName.toUpperCase());
    currentEditColumn.classList.remove(oldName.toLowerCase());
    currentEditColumn.classList.add(newName.toLowerCase());
    // Update tasks that belong to this column
    arrayOfTasks = arrayOfTasks.map(task => {
      if (task.category === oldName) {
        return { ...task, category: newName.toUpperCase() };
      }
      return task;
    });
    addDataToLocalStorageFrom(arrayOfTasks);
...
// Event listener for deleting the column via the modal
document.getElementById("deleteColumnButton").onclick = function () {
  if (confirm("Are you sure you want to delete this column? All tasks in this column will be deleted.")) {
    let colName = currentEditColumn.getAttribute("data-category");
    arrayOfTasks = arrayOfTasks.filter(task => task.category !== colName);
    addDataToLocalStorageFrom(arrayOfTasks);
    currentEditColumn.remove();
    document.getElementById("editColumnModal").classList.add("hidden");
    currentEditColumn = null;
  }
};

// Event listener for closing the edit column modal
document.getElementById("closeEditColumnModal").onclick = function () {
  document.getElementById("editColumnModal").classList.add("hidden");
  currentEditColumn = null;
};
